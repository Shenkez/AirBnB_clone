...
class HBNBCommand(cmd.Cmd):
    ...
    def do_show(self, arg):
        ...
        data_b = models.storage.all()
        if not len(arg_list):
            print("** class name missing **")
        elif (arg_list[0] not in HBNBCommand.__class_lst.keys()):
            print("** class doesn't exist **")
        elif len(arg_list) == 1:
            print("** instance id missing **")
        elif "{}.{}".format(arg_list[0], arg_list[1]) not in data_b:
            print("** no instance found **")
        else:
            print(data_b["{}.{}".format(arg_list[0], arg_list[1])])
